...
export default function TabBar({ streak = 0 }) {
  return (
    <nav
      className="bg-surface/95 glass border-t border-line flex items-center justify-around shrink-0"
    ...
    >
      {TABS.map((tab) => (
        <NavLink
          ...
          to={tab.to}
          className={({ isActive }) =>
            `relative flex flex-col items-center gap-1 py-2 px-5 transition-colors ${
            ...
            <>
              <span className="relative">
                ...
                {tab.label === 'Today' && streak > 0 && (
                  <span className="absolute -top-0.5 -right-2.5 w-4 h-4 bg-accent rounded-full flex items-center justify-center">
                    <span className="text-[9px] font-bold text-white leading-none">{streak}</span>
                  </span>
                )}
              </span>
